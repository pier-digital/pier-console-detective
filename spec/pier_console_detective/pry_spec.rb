...
RSpec.describe "PierConsoleDetective::Pry" do
  let(:fresh_binding) {
    local_a = 123
    binding
  }
  ...
  it "calls log command when before_eval is called" do
    ...
    expect(PierConsoleDetective::Utils).to receive(:log_command).with("puts '#{random_string}'")
    repl.send_input("puts '#{random_string}'")
  end

  it "calls log command when after_session is called" do
    expect(PierConsoleDetective::Utils).to receive(:log_command).with("exit", immediately: true)
